...
namespace AdventOfCode2017.Solutions
{
    class Day10Problem2 : ISolution
    {
        public string GenerateAnswer(string input)
        {
            ...
            var bytes = blocks.Select(it => this.XorCombineList(it)).Select(it => it.ToString("x2"));
            return string.Join(string.Empty, bytes);
        }

        private void ApplyValuesToArray(byte[] values, List<byte> updatedValues, int position)
        {
        ...
        private byte XorCombineList(List<byte> input)
        {
